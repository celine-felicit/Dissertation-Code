...
where where p is the probability of the dependent variable, α is the intercept, β is the coefficient for each independent variable, and γdyad_id represents the fixed effect for each dyad.
...
where where p is the probability of the dependent variable, α is the intercept, β is the coefficient for each independent variable, and udyad_id∼N(0,σ2) represents the random effect for each dyad.

  ### *How do I include the equations in the text?*
  ### *correct introduction of random and fixed effects in the equation?*
  
An overall comparison was made between the effects of conflict characteristics on the provision of external support within and between conflicts, with the statistically significant characteristics in the models for the provision of troop support and intelligence being compared in detail. These models were chosen for detailed comparison based on the high correlation between conflict characteristics and the provision of the corresponding support in either the FE or RE models. They further allow for a more detailed comparison of the correlation with various conflict characteristics between the provision of direct or indirect support. Coefficients were exponentiated for easier comparison and despite discussions on redefining statistical significance (Benjamin et al., 2018), this report rejects the null hypothesis that the population's estimated coefficient is zero when p <.05.
  ### *Correct to speak of coefficient here or would this need to be log odds ratio as we're using logistic models?*
Multicollinearity was monitored throughout the analysis using variance inflation factors (VIF) and all models were tested against standard regression assumptions to ensure validity and reliability. However, as most predictors were categorical, the linearity assumption did not apply to them and homoscedasticity and normality of residuals were not relevant due to the logistic nature of the models. For FE models the Adjusted Pseudo R-Squared was used to assess model fit. The Bayesian Information Criterion (BIC) and Akaike Information Criterion (AIC) were used to evaluate the RE models' goodness-of-fit; with lower values denoting a better fit (Gellman and Hill, 2007).
...
The provision of external support has risen, with indirect support being the most common (Figure 4) and supporters providing three forms of support on average (Median: 4). The provision of support in a coalition has increased over time (Appendix 4).
...
Within conflicts, as captured by the FE models (Table 4), the provision of external support is mostly correlated with the post-Cold War and post-9/11 periods, as well as conflict intensity. Troop support is particularly notable, with all significant correlations reaching high levels of statistical significance. While random effects suggest that indirect support is more strongly tied to conflict characteristics, FE highlight the temporal dimension, with direct support — troop presence and troop support — being significantly correlated to the era in which the conflict occurs. Conflict intensity and duration are correlated with the provision of indirect external support within a conflict but not with the provision of direct support.

  ###*add more Findings so that all variables are covered?*

Model-specific results further illustrate these patterns: troop support is most strongly correlated to coalition involvement and conflict type across both models, whereas intelligence provision and weapons supply show broader but less consistent correlations with conflict characteristics. In the RE model (Table 5), interstate conflicts are associated with higher odds (504.151) of receiving troop support compared to intrastate conflicts, holding all other conflict characteristics constant. The *average(?)* odds of troop support provision for support provided in a coalition are  33712.247 larger than the average odds of bilateral support, controlling for all other conflict characteristics included in the model. In contrast, the FE model (Table 6) finds that internationalised intrastate conflicts are correlated to the provision of troop support, with an odds difference of *11786432631969029355286460062.000* compared to intrastate conflicts, holding all other factors constant. The strong correlation between coalition support and the provision of troop support remains in the FE model, but its effect is much larger (odds = *2202685333377795584.000*) than in the random effects model. However, the FE model also suggests that conflicts occurring after 9/11 and after the Cold War are negatively correlated with the provision of troop support, with reductions in the average odds of  0.032 and 0.000 compared to the average odds before 9/11 and during the Cold War, respectively - correlations that are non-significant in the RE model.

For intelligence support, both models find that the average odds for wars are smaller than that for minor armed conflicts to receive this form of aid (odds = 0.486 in RE; 0.325 in FE), holding all other factors constant. Both models further indicate that conflicts occurring after the Cold War are correlated with the provision of intelligence, although the effect is stronger in the FE model (odds = 8.413) than in the RE model (odds = 1.631). The RE model identifies regional differences, with conflicts in the Americas having an odds of intelligence provision that is 124.034 larger than the odds of conflicts in Europe, controlling for incompatibility, type, intensity, region, duration, 9/11, Cold War, and coalition support. It further indicates that conflicts about government have a odds of being provided intelligence that is 0.140 smaller than the odds of conflicts about territory, controlling for type, intensity, region, duration, 9/11, Cold War, and coalition support. While the RE model suggests that intelligence provision is correlated to the time period after 9/11 (odds = 3.049) and coalition support (odds = 2.401), holding all other variables constant, these effects do not emerge as statistically significant in the FE model. Notably, only the FE model finds that the odds of receiving intelligence support increases with the duration of the conflict (odds = 1.118 per year).
...
All models were tested against standard regression assumptions to ensure validity and reliability. The data, sourced from the UCDP, was deemed appropriate for addressing the research questions, as it comprehensively captures armed conflict characteristics and external support. Although, empirical datasets rarely meet all theoretical criteria perfectly (Gellman and Hill, 2007). Multicollinearity was examined using variance inflation factors (VIFs), with all values below 5, indicating no significant collinearity issues (O'Brien, 2007). As most predictors were categorical, the linearity assumption did not apply to them. Linearity was tested using the `allEffects()` function for the only continuous predictor, cumulative duration, which showed a linear relationship with the log odds of the dependent variable in all RE models. [*add how assumption was tested in fixed effects*] The independence of errors assumption was not met due to multiple observations per conflict and dyad. Homoscedasticity and normality of residuals were not relevant due to the logistic nature of the models. The binary nature of the dependent variable confirmed the appropriateness of logistic regression. 

Model fit assessment reveals differences both across different forms of external support and between the FE and RE models. Among the FE models (Table 4), troop support has the highest adjusted pseudo R-squared (0.811), indicating a strong explanatory power, while troop presence has the lowest (0.009), suggesting that conflict characteristics do little to explain variations in the provision of troop presence within conflicts. Intelligence (0.208), funding (0.179), access to territory (0.170), and weapons (0.158) also show relatively better model fit compared to the other forms of support, whereas access to infrastructure (0.045), training and expertise (0.030), and materiel and statistics (0.071) exhibit weaker fits. BIC values further indicate that troop support (364.7) has the best trade-off between model fit and complexity among the FE models, while other models — especially those related to access to infrastructure (1074.7) and funding (1128.9) — show higher BIC values, indicating weaker model performance. For the RE models (Table 3), AIC and BIC provide insights into model performance across different forms of support. Troop support again exhibits the lowest AIC (86.1) and BIC (165.3), suggesting the best model fit among the RE models. In contrast, access to infrastructure has a particularly high AIC (1255.9) and BIC (1335.1), indicating weaker explanatory power. Intelligence (AIC = 923.7, BIC = 1002.9) and funding (AIC = 1313.1, BIC = 1392.3) show moderate model fit, aligning with their relatively strong performance in the FE models. When comparing the FE and RE models, the FE model for troop support has an exceptionally high adjusted pseudo R-squared (0.811), reinforcing its strong explanatory power. However, the RE model for troop support has a much lower BIC (165.3) compared to the FE BIC (364.7), suggesting that the RE approach may be more efficient in accounting for between-conflict variations. For other forms of support, the FE models generally exhibit stronger explanatory power (higher pseudo R-squared values) but tend to have higher BIC values, reflecting greater model complexity. The RE models, in contrast, often have lower AIC and BIC values, indicating better model fit in terms of parsimony, particularly for troop support and intelligence.
...
Findings suggest that coalition support (H3) is significantly associated with troop support, access to infrastructure, intelligence, and access to territory in the RE model. However, in the FE model, it only remains statistically significant for troop support. This suggests that coalition dynamics play a more prominent role in shaping the types of external support provided across different conflicts than within individual conflicts.
...
Overall, while both models provide insights into the determinants of external support, RE models perform better in explaining broader cross-conflict patterns, whereas FE models capture within-conflict variation more effectively. The weaker correlation of troop presence with conflict characteristics in both models suggests that direct intervention may be driven by factors beyond conflict characteristics alone, such as diplomatic or geopolitical considerations. 
...
ggplot(incompatibility_summary, aes(x = year, y = conflict_count, linetype = incompatibility)) +
  ...
  labs(
    title = "Appendix 3: Evolution of state-based conflicts by incompatibility (1975–2017)",
    x = "Year",
    y = "Number of conflicts",
...
ggplot(coalition_summary, aes(x = year, y = conflict_count, color = ext_coalition, linetype = ext_coalition)) +
  geom_line(linewidth = 1) +
  labs(
    title = "Appendix 4: Evolution of external support: bilateral vs coalition",
    x = "Year",
    y = "Number of conflicts",
