...
## Research Design
The study aims to test several hypotheses regarding the relationship between conflict characteristics and external support. These hypotheses are grouped based on the type of relationship they examine, focusing on whether conflict characteristics influence the provision of external support, the form of support provided, and the impact of support coalitions on the type of assistance. These conflict characteristics are considered to be strategical factors influencing the provision of external support. Thereby, this study aims to examine whether external supporters follow a 'cold', rational approach to external support. The hypotheses are as follows:
...
b)	Territorial incompatibility affects whether external support is provided
...
e)	Whether a conflict is taking place before or after 9/11 affects whether external support is provided.
f)	Whether a conflict is taking place during or after the Cold War affects whether external support is provided.
...
b)	Territorial incompatibility affects what kind of external support is provided.
...
e)	Whether a conflict is taking place before or after 9/11 affects what kind of external support is provided.
f)	Whether a conflict is taking place during or after the Cold War affects what kind of external support is provided.
...
where where p is the probability of the dependent variable, $\alpha$ is the intercept, $\beta$ is the coefficient for each independent variable, and $gamma_{\text{dyad\_id}$ represents the fixed effect for each dyad.
...
where where p is the probability of the dependent variable, $\alpha_i$ is the intercept varying between dyads (i), $\beta$ is the coefficient for each independent variable.
  
As both RE and FE models for the overall provision of support, troop support, and foreign troop presence showed non-convergence, they were refitted as Poisson Pseudo Maximum Likelihood (PPML) models. The PPML regression models were fitted using the same independent variables as the logistic regression models, with the dependent variable being the count of the respective form of external support, using the following equations:
...
\right)
$$
where $\gamma_i$ represents the dyad-specific fixed effects. The exponentiation is key to PPML, as the model estimates log-linear coefficients using a Poisson-based pseudo-likelihood, even when the dependent variable is not truly count data. Different to the RE model, $\gamma_i$ is treated as a parameter to be estimated, rather than a random draw from a distribution.
...
The models for the provision of external support (Table 2a; 2b) show no statistically significant results. However, the results of the regression analyses for individual forms of support reveal distinct patterns in the relationship between conflict characteristics and the provision of external support, with notable differences between the RE and FE models.
...
Within conflicts, as captured by the FE models (Table 4), the provision of external support is overall less correlated to conflict characteristics than in the comparison between conflicts (RE). The era post-Cold War, as well as conflict intensity, show the most significance across FE models. However, conflict intensity, as well as duration are correlated with the provision of indirect external support within a conflict but not with the provision of direct support. The provision of weapons and intelligence are correlated with the most conflict characteristics, with conflict intensity, duration, and the post-Cold War era beign significant in both. The provision of troop support is only correlated to the type of conflict, while troop presence is correlated to the Post-Cold war and post-9/11 eras. Access to infrastructure is not correlated to any conflict characteristics and the provision of materiel and statistics, training and expertise, funding and access to territory are only correlated to one or two conflict characteristics.
...
While the models examining H1 shows no statistically significant results, regression analyses for individual forms of support reveal distinct patterns in the relationship between conflict characteristics and the provision of external support, with notable differences between the RE and FE models. Therefore H1 cannot be completely rejected but the results rather indicate that the provision of external support is not only correlated to conflict characteristics but that it further may be dependent on the form of support provided. 

The RE models show that the provision of indirect support is more correlated to conflict characteristics than direct support, with intelligence provision being the most sensitive to these factors. This finding aligns with existing literature suggesting that indirect forms of support are often more prevalent in contemporary conflicts (*INSERT REFERNCE*). Especially the provision of intelligence is strongly correlated with conflict characteristics, which could suggest that external actors tailor intelligence support to specific conflict dynamics rather than adopting a one-size-fits-all approach. This supports the notion that intelligence-sharing plays a crucial role in modern conflict interventions, allowing external supporters to shape conflict outcomes strategically (Schultz, 2010; Bapat, 2011; Sawyer, Cunningham and Reed, 2017).

For H2, which explores whether conflict characteristics influence the type of external support provided, results reveal that conflict intensity (H2a) is significantly correlated to the provision of weapons, materiel and statistics, funding, and intelligence in both FE and RE models. This aligns with the argument that higher-intensity conflicts are more likely to receive external support due to concerns about spillover effects and escalation (Cunningham, 2010; Gregory, 2000). However, intensity does not appear to significantly impact the provision of troop support, suggesting that external actors may not relate the intensity of a conflict to increased levels of direct support. 

Besides intensity, the provision of access to infrastructure, weapons, materiel and statistics, training and expertise, funding, and access to territory show consistent correlations with conflict duration (H2c), the type of conflict (H2d), and whether a conflict is taking place after 9/11 (H2e) or after the Cold War (H2f) in the RE models. Particularly internationalised intrastate conflicts, is strongly correlated with the provision of external support. Within a conflict, conflict intensity, duration, and the post-Cold War era seem to be more correlated with the provision of indirect external support. These findings are consistent with prior research suggesting that the Cold War significantly shaped patterns of external intervention. During this period, superpowers engaged in proxy conflicts, fueling insurgencies and prolonging wars, whereas the post-Cold War era saw a shift towards more selective and strategic forms of external support (Testerman, 2015; Kalyvas and Balcells, 2010 in Roberts, 2019). The significance of the periods after the Cold War and 9/11, and conflict intensity in both FE and RE models, suggests that broader geopolitical shifts and conflict dynamics play a crucial role in shaping external support provision, whether within one ongoing or between different conflicts. This divergence highlights the complexity of external support dynamics and suggests that different forms of support may be influenced by different factors.

Territorial incompatibility (H2b) on the other hand is not significantly correlated with the provision of most external support, suggesting that this factor plays a less prominent role in shaping external intervention strategies. The lack of significant correlations between territorial incompatibility with external support provision is surprising, as previous research has highlighted the importance of this factor in shaping conflict dynamics and external interventions (Cunningham, 2010; Gregory, 2000). *What does this mean? Interpret!* 
The absence of a significant relationship between cumulative conflict duration and the provision of support challenges assumptions that protracted conflicts necessarily attract sustained external intervention.

Model-specific results further illustrate these patterns: troop presence is most strongly correlated to the post-Cold War and post-9/11 eras (FE), and conflict intensity (RE), whereas intelligence provision and weapons supply show broader but less consistent correlations with conflict characteristics. 

In the RE model (Table 5), the average IRR of foreign troop presence is 2.52 units higher for wars compared to minor armed conflicts, holding all other conflict characteristics constant. The average IRR is further 10.18 units higher for conflicts taking place after 9/11 compared to conflicts before 9/11, controlling for all other variables. 
...
tab_model(
  ...
  dv.labels = c("Troop presence (PPML)", "Intelligence"),
  title = "Random Effects Logistic and PPML Regression Results: External Support Determinants"
)
```
...
For intelligence support, both models find that the odds for wars are lower than that for minor armed conflicts to receive this form of aid (odds = 0.44 in RE; 0.33 in FE), holding all other factors constant. Both models further indicate that conflicts occurring after the Cold War are correlated with the provision of intelligence, although the effect is stronger in the FE model (odds = 8.41) than in the RE model (odds = 6.4). Both models find that the odds of receiving intelligence support increase with each year in conflict duration (by 1.06 in RE and 1.12 in FE models). While the RE model suggests that intelligence provision is correlated to the time period after 9/11 (odds = 8.27) and coalition support (odds = 4.49), holding all other variables constant, these effects do not emerge as statistically significant in the FE model.

Findings suggest that coalition support (H3) is significantly associated with access to infrastructure, funding, intelligence, and access to territory in the RE model. However, in the FE model, it shows no statistical significance at all. This suggests that coalition dynamics play a more prominent role in shaping the types of external support provided across different conflicts than within individual conflicts.

While the correlation to different conflict characteristics of troop presence and troop support might seem counterintuitive at first, this could indicate an escalation in the provision of external support with the former being a precondition for the latter. Conflict intensity (RE), and the post-Cold war and post-9/11 eras (FE) might be first incentives for external supporters to provide more direct support but once troops are already present, the type of conflict might then become a more important factor in the decision to provide troop support.
...
However, this study is not without limitations. The analysis is based on a dataset that primarily focuses on state-based conflicts, which may not fully capture the complexities of non-state actors and their interactions with external support. The analysis is further limited by the availability of data on external support, which may not encompass all forms of assistance provided to conflict parties. The exclusion of regional variations in conflict characteristics and external support may further limit the generalisability of the findings, as well as their interpretation from a criminological perspective. Tobler's law (Tobler, 1970), "everything is related to everything else, but near things are more related than distant things" suggests that geographic proximity may play a significant role in shaping the dynamics of external support. Not controlling for geographic proximity might thereby ignore important contextual factors that influence the provision of external support. It further limits the interpretation from a 'hot' and 'cold' support perspective. While intensity might trigger a 'forward panic' (Collins, *year* in Rafter, 2016), the absence of adequate control variables of more emotional factors like shared cultural ties, as well geographic proximity, which might have an effect on the perceived urgency of intervention, makes it impossible to assess whether emotional elements play a role in the provision of external support. Further research should examine the emotional factors that play a role in supporters' decision to get involved. This could further explore how the provision of support affects the continuance of conflict and mass violence and whether this differs between 'hot' and 'cold' support. The inability to test recipient and provider characteristics further limits this study and warrants further research into the role of these factors in shaping external support provision.

The study's limitations further include the potential for reverse causality, as external support may prolong conflicts rather than resolve them. Interventions, whether direct (troops) or indirect (weapons, logistics, training), frequently prolong conflicts by altering the cost-benefit calculations of belligerents (Fearon, in Testerman, 2015; Lacina, 2006). Further, while the use of different regression models was deemed necessary due to non-convergence, this approach introduces inconsistencies in the interpretation of results and makes comparison between models difficult. Finally, the study's reliance on quantitative data may overlook important qualitative aspects of external support and its effects on conflict dynamics. Future research should therefore enhance this study by incorporating qualitative data and case studies to provide a more comprehensive understanding of the relationship between external support and conflict dynamics. 
...
While diplomacy *remains* the preferred approach to resolving conflicts, the persistence of aggressive state actors — such as Russia's actions in Ukraine — challenges the viability of purely pacifist strategies. Historical patterns suggest that military interventions can sometimes bring stability but may also exacerbate conflicts, as seen in Iraq and Afghanistan (*INSERT REFERENCE*). Given that external support often takes indirect forms (e.g., intelligence-sharing, training, and funding), policymakers must consider the long-term implications of such interventions. The interconnected nature of modern conflicts underscores the necessity for early mediation, intelligence-sharing, and conflict prevention strategies. This is particularly relevant in the context of sustainable development policies, as conflict and fragility pose significant barriers to economic and social progress (Le, Bui, and Uddin, 2022). To mitigate the risk of prolonged violence, policymakers in conflict-affected regions should adopt inclusive recovery strategies that address the root causes of instability (World Bank and United Nations, 2018). If external support is provided selectively to prolong conflicts rather than resolve them, it risks perpetuating cycles of violence. These findings reinforce the importance of understanding the strategic logic behind external support provision, both from a policy and criminological perspective. As global security dynamics continue to evolve, future research should explore case studies of external support to assess its long-term impact on conflict resolution and stability.
  ...
  ###*What else could be done to bring this discussion into Criminology?*
