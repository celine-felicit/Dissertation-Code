...
Model-specific results further illustrate these patterns: troop support is most strongly correlated to coalition involvement and conflict type across both models, whereas intelligence provision and weapons supply show broader but less consistent correlations with conflict characteristics. In the RE model (Table 5), interstate conflicts are associated with higher odds of receiving troop support (log odds = 5.905) compared to intrastate conflicts, holding all other conflict characteristics constant. The average log odds of troop support provision for support provided in a coalition are 9.650 larger than the average log odds of bilateral support, controlling for all other conflict characteristics included in the model. In contrast, the FE model (Table 6) finds that internationalised intrastate conflicts are correlated to the provision of troop support, with a log odds difference of 64.637 compared to intrastate conflicts, holding all other factors constant. The strong correlation between coalition support and the provision of troop support remains in the FE model, but its effect is much larger (log odds = 42.236) than in the random effects model. However, the FE model also suggests that conflicts occurring after 9/11 and after the Cold War are negatively correlated with the provision of troop support, with reductions in the average log odds of 3.443 and 13.187 compared to the average log odds before 9/11 and during the Cold War, respectively — correlations that are non-significant in the RE model.  

For intelligence support, both models find that the average log odds for wars are smaller than that for minor armed conflicts to receive this form of aid (log odds = -0.808 in RE; -1.123 in FE), holding all other factors constant. Both models further indicate that conflicts occurring after the Cold War are correlated with the provision of intelligence, although the effect is stronger in the FE model (log odds = 2.130) than in the RE model (log odds = 1.592). The RE model identifies regional differences, with conflicts in the Americas having a log odds of intelligence provision that is 4.121 larger than the log odds of conflicts in Europe, controlling for incompatibility, type, intensity, region, duration, 9/11, Cold War, and coalition support. It further indicates that conflicts about government have a log odds of being provided intelligence that is 1.831 smaller than the log odds of conflicts about territory, controlling for type, intensity, region, duration, 9/11, Cold War, and coalition support. While the RE model suggests that intelligence provision is correlated to the time period after 9/11 (log odds = 2.410) and coalition support (log odds = 1.076), holding all other variables constant, these effects do not emerge as statistically significant in the FE model. Notably, only the FE model finds that the log odds of receiving intelligence support increases with the duration of the conflict (log odds = 0.111 per year).
